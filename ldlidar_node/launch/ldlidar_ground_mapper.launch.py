#!/usr/bin/env python3
"""
Launch file for LDLidar with Ground Mapper

Launches:
1. Lifecycle manager
2. Robot state publisher
3. LDLidar component node (in container)
4. Ground mapper node

This launch file starts the lidar and the ground scanning mapper
that generates a 2D topological map based on drone velocity.
"""
...
def generate_launch_description():
    ...
    declare_drone_velocity_cmd = DeclareLaunchArgument(
        'drone_velocity_y',
        default_value='1.0',
        description='Drone velocity in Y-direction (m/s)'
    )
    ...
    drone_velocity_y = LaunchConfiguration('drone_velocity_y')
    ...
    ground_mapper_node = Node(
        ...
        parameters=[
            params_file_launch,
            {'drone_velocity_y': drone_velocity_y}
        ]
    )
